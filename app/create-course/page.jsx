...
import { getCourseImage, fetchPexelsImage } from "@/lib/imageService";
...
const CreateCourse = () => {
  ...
  const saveCourseLayoutDb = async (courseLayout) => {
    ...
    // Try to fetch a relevant image for the course
    let courseBanner = '/course-cover.svg'; // default fallback
    
    try {
      // First, try to get a category-specific image
      const tempCourse = {
        category: userCourseInput?.category,
        courseOutput: { course: courseLayout.course }
      };
      
      const categoryImage = getCourseImage(tempCourse, {
        preferYoutube: false,
        fallbackToCategory: true
      });
      
      // If we have a category image that's not the default, use it
      if (categoryImage && categoryImage !== '/course-cover.svg') {
        courseBanner = categoryImage;
      }
      
      // Optionally, try to fetch from Pexels API if enabled
      if (process.env.NEXT_PUBLIC_PEXELS_API_KEY && userCourseInput?.topic) {
        try {
          const pexelsImage = await fetchPexelsImage(userCourseInput.topic, categoryImage);
          if (pexelsImage && pexelsImage !== categoryImage) {
            courseBanner = pexelsImage;
          }
        } catch (error) {
          console.warn('Pexels API failed, using category image');
        }
      }
    } catch (error) {
      console.warn('Failed to fetch course image, using default');
    }
